	...
	$.ajax({
		...
		success : function(likeAjax){
			...
			if(likeAjax.likeResult == 1){
				...
				img = "<img src='/resources/images/blue_check.png' style='width: 30px'>";
			}else{
				console.log("insert"+img);
				img = "<img src='/resources/images/check.png' style='width: 30px'>";
			}
...
function filter(job_field, username){
	...
	$.ajax({
		...
		success : function(data){
			if(data.filterCompPickInfoList != null){
				console.log("아자");
				var filter = "";
				var main="<div class='js-load2'>"+
					  "<ul class='lists'>";
				$.each(data.userComPpList, function(index1, cList){
					$.each(data.filterCompPickInfoList, function(index2, uList){
						if(cList.username == uList.username){
			               ...
			               $.each(data.listLike, function(index3, lList){
			                  ...
			                  if(lList.pick_code == uList.pick_code){		
			                	  filter +="<div style='height: 23%;'>"+
				                        "<a href='#' id='like"+index2+"' onclick=like('jung',"+uList.pick_code+","+index2+")><img src='/resources/images/check.png'style='width: 30px'>"+
				                        "</a>"+
				                     "</div>";
			                	  check = 1;
			                  }
			               ...
			               if(check == 0){
			               filter += "<div style='height: 23%;'>"+
			                  	"<a href='#' id='like"+index2+"' onclick=like('jung',"+uList.pick_code+","+index2+")><img src='/resources/images/blue_check.png'style='width: 30px'>"+
			                     "</a>"+
			                  "</div>";
			               }
			            filter += "</div>";
				...
				(function (){
					filterMore('.js-load2', '4');
					console.log("돼라");
				    $(".button").on("click", function () {
				    	console.log("buttn");
				    	filterMore('.js-load2', '4', '.js-btn-wrap');
				    })
				})()
				$('#pickSearch').val('');
			}else{
				$('#filtering').html('<div></div>');
			}
		}
	})
...
<div id="content">
	<div class="container">
		<h2>PICK ME!</h2>
		<p>핫이슈! 채용공고 (가장 최신으로 업데이트된 채용공고 입니다.)</p>
		...
		<div class="row">
			<div class="btn-group-vertical col-sm-2" style="float: left;">
				<input type="button" id="manage" class="btn btn-primary" value="나의 채용공고" onclick="filter(this.value, 'jung')">
						...
						<c:if test="${userComPpList.get(i).getUsername() eq compPickList.get(j).getUsername()}">
							...
							<div id="company_pick"> 
									...
									<c:if test="${listLike.getPick_code() eq  compPickList.get(j).getPick_code()}">
										<div style="height: 23%;">
											<a href='#' id="like${j}" onclick="like('jung', ${compPickList.get(j).getPick_code()}, ${j})">
												<img src="/resources/images/check.png"style="width: 30px">
											</a>
								...
								<c:if test="${likeCheck != 1}">
									<div style="height: 23%;">
										<a href='#' id="like${j}" onclick="like('jung', ${compPickList.get(j).getPick_code()}, ${j})">
											<img src="/resources/images/blue_check.png"style="width: 30px">
										</a>
										<c:set var="likeCheck" value="0"/>
									</div>
								</c:if>
